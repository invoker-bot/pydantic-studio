...
`METADATA` and the sdist `PKG-INFO`, confirms the wheel filename, wheel
`.dist-info` identity, structure files, purelib flag, compatibility tag,
...
present in the source archive. The
