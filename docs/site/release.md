...
`py.typed` marker, packaged web static bundle files, `RECORD` entries, and
console script entry points match `[project.scripts]`, then confirms
`pyproject.toml`, the declared README file, the sdist `py.typed` marker, the
sdist web static bundle files, and every explicit `source-include` entry are
