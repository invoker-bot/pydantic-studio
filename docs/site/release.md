...
The release metadata verifier checks the package Name, Version, and
Requires-Python plus every declared Project-URL in both the wheel `METADATA`
and the sdist `PKG-INFO`, then confirms every explicit `source-include` entry
is present in the source archive. The Changelog, Security, and Contributing
URL targets must also be listed in `source-include`.
